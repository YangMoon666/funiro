window.onload = function () {
    ...
    function documentActions(e) {
        ...
        if (targetElement.classList.contains('cart__body_show_more_btn')) {
            var showBlocks = document.querySelectorAll('.scroll_cart');
            for (let key of showBlocks) {
                key.classList.toggle('_active');
                let content = key;
                if (content.style.maxHeight) {
                    content.style.maxHeight = null;
                } else {
                    let one = document.querySelector('.cart_block');
                    console.log(one.offsetHeight);
                    content.style.maxHeight = one.offsetHeight + 'px';
                }
            }
    ...
    // шапка фиксация при открытом поиске
